package edu.mike.frontend.taskapp.presentation.viewmodel
        ...
        class TaskViewModelTest {
            ...
            /**
             * Tests the getTaskById function to ensure it updates task state and selectedTask with success result.
             */
            @Test
            fun `getTaskById should update task state and selectedTask with success result`() = runTest {
                // Arrange
                coEvery { repository.findTaskById(1L) } returns Result.success(mockTask)

                // Act
                viewModel.getTaskById(1L)
                advanceUntilIdle()

                // Assert
                assertTrue(viewModel.task.value is TaskState.Success)
                val successState = viewModel.task.value as TaskState.Success
                assertEquals(mockTask, successState.task)
                assertEquals(mockTask, viewModel.selectedTask.value)
                coVerify { repository.findTaskById(1L) }
            }

            /**
             * Tests the getTaskById function to ensure it updates to error state on failure.
             */
            @Test
            fun `getTaskById should update to error state on failure`() = runTest {
                // Arrange
                val exception = Exception("Test error")
                coEvery { repository.findTaskById(1L) } returns Result.failure(exception)
                ...
                // Assert
                assertTrue(viewModel.task.value is TaskState.Error)
                val errorState = viewModel.task.value as TaskState.Error
            ...
            /**
             * Tests that getTaskById goes through loading state.
             */
            @Test
            fun `getTaskById should set loading state before getting result`() = runTest {
                // Arrange
                coEvery { repository.findTaskById(1L) } returns Result.success(mockTask)
                ...
                // Assert loading state before advancing
                assertTrue(viewModel.task.value is TaskState.Loading)

                // Advance and complete the coroutine
                advanceUntilIdle()
             ...
             */
            @Test
            fun `initial state should be empty and initial`() {
                assertEquals(emptyList<Task>(), viewModel.taskList.value)
                assertNull(viewModel.selectedTask.value)
                assertTrue(viewModel.task.value is TaskState.Initial)
            }
        }